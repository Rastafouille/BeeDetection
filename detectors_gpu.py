...
class Detector1(object):
    ...
    def detect(self, gpu_currentframe, gpu_previousframe):
        gpu_current=gpu_currentframe
        gpu_previous=gpu_previousframe
        gpu_previous_gray = cv.cuda.cvtColor(gpu_previous, cv.COLOR_BGR2GRAY)
        if self.DEBUG :
        ...
        contours_new, hierarchy = cv.cuda.findContours(gpu_dilate2,cv.RETR_TREE,cv.CHAIN_APPROX_SIMPLE)
        gpu_frame_contours=gpu_current
        cv.cuda.drawContours(gpu_frame_contours, contours_new, -1, (0,0,255), 2)
...
#    contours_old=contours_new
        ...
        return (centers,gpu_frame_contours,rect)   
